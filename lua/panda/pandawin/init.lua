...
local function get_tab_windows()
    ...
    for _, tab in ipairs(vim.fn.gettabinfo()) do
        if tab.tabnr == vim.fn.tabpagenr() then tab_wins = tab.windows end
    end
    return tab_wins
end
...
local function backup_win_statusline()
    ...
    if choose_win ~= nil then
        ...
        if choose_index ~= nil and choose_index ~= cur_win and choose_index <=
            #wins then
...
local function choose_win() backup_win_statusline() end
